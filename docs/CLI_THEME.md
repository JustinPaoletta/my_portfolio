...
This document explains the interactive CLI theme behavior, controls, and command model.

## Overview

The CLI theme presents the portfolio as a fullscreen terminal-style interface with:
...
On app load in CLI mode, output starts with minimal pre-rendered text to keep the active prompt near the top.
...
In CLI mode, the top navbar is hidden.

## Theme Switcher in CLI
...
The CLI supports staged input and Enter-to-run:
...
`exit`/`quit` switches from CLI theme back to the app default theme (`engineer`).
