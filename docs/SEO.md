...
| Sitemap and robots              | `plugins/vite-plugin-sitemap.ts`, `public/robots.txt`, `dist/sitemap.xml`, `dist/robots.txt` | Helps engines discover URLs and understand crawl policy                                                   |
...
- writes `dist/robots.txt` with the correct sitemap URL using `public/robots.txt` as the source template
- emits only canonical URLs plus accurate `lastmod` values for each entry
