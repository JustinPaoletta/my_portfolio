...
- Cosmic and engineer hero posters now fade out over the live WebGL canvas instead of disappearing in a single frame, eliminating the poster-to-3D flicker. Live canvas snapshots are pre-decoded before swapping the poster `src`, and the fade is skipped when `prefers-reduced-motion` is enabled.
